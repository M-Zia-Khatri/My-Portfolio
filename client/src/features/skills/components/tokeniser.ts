import type { Token } from "../types";

export const TOKEN_COLORS: Record<string, string> = {
  keyword: "#c792ea",
  tag: "#f07178",
  string: "#c3e88d",
  comment: "#546e7a",
  fn: "#82aaff",
  type: "#ffcb6b",
  default: "#cdd6f4",
};

const PATTERNS: Array<[RegExp, string]> = [
  [/\/\/.*$/, "comment"],
  [/#.*$/, "comment"],
  [/"[^"]*"|'[^']*'|`[^`]*`/, "string"],
  [
    /\b(import|export|default|from|const|let|var|type|interface|class|function|return|extends|public|private|new|if|else|<?php)\b/,
    "keyword",
  ],
  [/\b(string|number|boolean|void|any|JsonResponse|array|int)\b/, "type"],
  [/<\/?[a-zA-Z][\w-]*/, "tag"],
  [/[a-zA-Z_][\w]*(?=\s*\()/, "fn"],
];

export function tokenise(line: string): Token[] {
  const parts: Token[] = [];
  let rem = line;
  let pos = 0;

  while (rem.length > 0) {
    let hit = false;
    for (const [re, kind] of PATTERNS) {
      const m = rem.match(re);
      if (m && m.index !== undefined) {
        if (m.index > 0) {
          const text = rem.slice(0, m.index);
          parts.push({ text, color: TOKEN_COLORS.default, id: pos });
          pos += text.length;
        }
        parts.push({ text: m[0], color: TOKEN_COLORS[kind] ?? TOKEN_COLORS.default, id: pos });
        pos += m[0].length;
        rem = rem.slice(m.index + m[0].length);
        hit = true;
        break;
      }
    }
    if (!hit) {
      parts.push({ text: rem, color: TOKEN_COLORS.default, id: pos });
      break;
    }
  }

  return parts;
}
