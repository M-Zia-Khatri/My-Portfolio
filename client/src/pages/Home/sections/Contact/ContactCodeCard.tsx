...
import CodeCard, { type CodeCardHandle } from "../Skills/CodeCard";
import TabScrollbarStyle from "../Skills/TabScrollbarStyle";
import { skills as ALL_SKILLS } from "../Skills/skills.data";
import type { Skill } from "../Skills/types";

// ── Curated list: code-only skills so the typewriter always fires ─────────────
const CONTACT_SKILLS = ALL_SKILLS.filter(
...
function ProgressRail({ autoIndex, isDone }: { autoIndex: number; isDone: boolean }) {
  return (
    <div className="flex items-center justify-center gap-[6px] py-2.5">
      {CONTACT_SKILLS.map((s, i) => (
        <motion.div
          ...
          className="h-[4px] rounded-full cursor-default"
        />
      ))}
    </div>
  );
